...
| **AI-grill** | Autonomous grill mode (triggered by `--afk` flag or `CLANCY_MODE=afk`). A devil's advocate agent answers clarifying questions using codebase, board, and web research. Same relentless energy as the human grill — does not accept its own vague answers. Single pass. |
