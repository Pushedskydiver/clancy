...
| **Setup** | Core role. Configuration and maintenance. Commands: `/clancy:init`, `/clancy:settings`, `/clancy:doctor`, `/clancy:update`, `/clancy:uninstall`, `/clancy:help`. |
...
| **Progress entry** | A line in `.clancy/progress.txt` recording a completed action (e.g. `DONE`, `PR_CREATED`, `REWORK`, `EPIC_PR_CREATED`). |
...
| **Grill phase** | The clarification step before brief generation. The strategist asks probing questions about scope, users, constraints, and edge cases to resolve ambiguity upfront. |
| **Human grill** | Interactive grill mode (default). The strategist interviews the human directly, with multi-round follow-up questions. |
| **AI-grill** | Autonomous grill mode (triggered by `--afk` flag or `CLANCY_MODE=afk`). A devil's advocate agent answers clarifying questions using codebase, board, and web research. Single pass. |
...
| Term | Definition |
|---|---|
| **Plan** | An implementation plan generated by `/clancy:plan` for a specific ticket. Describes the approach, files to change, and steps. Stored in `.clancy/plans/`. |
