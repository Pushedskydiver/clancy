/**
 * Linear — E2E test against real Linear API + GitHub sandbox repo.
 *
 * Creates a real Linear issue, runs the once orchestrator with Claude mocked
...
import { getGitHubCredentials, getLinearCredentials, hasCredentials } from '../helpers/env.js';
...
// ---------------------------------------------------------------------------
// Skip if credentials not available (need both Linear + GitHub)
// ---------------------------------------------------------------------------
...
describe.skipIf(!canRun)('E2E: Linear — full pipeline', () => {
  ...
  afterAll(async () => {
    if (prNumber) {
      await cleanupPullRequest('linear', prNumber).catch(() => {});
    }
    if (repo && ticketBranch) {
      cleanupBranch(repo.repoPath, ticketBranch);
    }
    if (ticket) {
      await cleanupTicket('linear', ticket.id).catch(() => {});
    }
    if (repo) {
      repo.cleanup();
    }
