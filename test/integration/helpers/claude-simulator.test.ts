...
describe('claude-simulator', () => {
  ...
  describe('simulateClaudeSuccess', () => {
    it('creates files, commits, and returns SHA', () => {
      repo = createTempRepo();
      const sha = simulateClaudeSuccess(repo.repoPath, 'TEST-1');

      expect(sha).toMatch(/^[a-f0-9]{40}$/);
      expect(existsSync(join(repo.repoPath, 'src', 'test-1.ts'))).toBe(true);
    });

    it('commit message matches conventional format', () => {
      repo = createTempRepo();
      simulateClaudeSuccess(repo.repoPath, 'PROJ-42');
    ...
    it('created files pass tsc --noEmit', () => {
      repo = createTempRepo();
      simulateClaudeSuccess(repo.repoPath, 'TEST-1');
    ...
    it('accepts custom files override', () => {
      ...
      expect(existsSync(join(repo.repoPath, 'src', 'custom.ts'))).toBe(true);
      expect(existsSync(join(repo.repoPath, 'src', 'test-1.ts'))).toBe(false);
    });
  });
  ...
  describe('simulateClaudeFailure', () => {
    it('creates files with errors and returns SHA', () => {
      repo = createTempRepo();
      const sha = simulateClaudeFailure(repo.repoPath, 'TEST-2');

      expect(sha).toMatch(/^[a-f0-9]{40}$/);
      expect(existsSync(join(repo.repoPath, 'src', 'test-2.ts'))).toBe(true);
    });

    it('created files fail tsc --noEmit', () => {
      repo = createTempRepo();
      simulateClaudeFailure(repo.repoPath, 'TEST-2');
  ...
  describe('createSequencedClaudeMock', () => {
    it('calls failure then success in sequence', () => {
      ...
      const mock = createSequencedClaudeMock([
        { type: 'failure', repoPath: repo.repoPath, ticketKey: 'TEST-3' },
        { type: 'success', repoPath: repo.repoPath, ticketKey: 'TEST-3' },
      ]);
